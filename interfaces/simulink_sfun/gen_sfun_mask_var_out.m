function [ ] = gen_sfun_mask_var_out(vars)
...
for i = 1:length(vars)
    ...
    if intvar
        gen_sfun_single_var_out(vars{i}.name,i-1,4,adim)
    else
        if strcmp(gendata.precision,'double')
            gen_sfun_single_var_out(vars{i}.name,i-1,0,adim)
        else
    ...
    function [] = gen_sfun_single_var_out(var_name,index,typeID,N)
        ...
        addl(['port_label( ''output'',' num2str(index+1) ', ''' var_name ' ' num2str(N) ' ' strType ' '')']);       
    end
